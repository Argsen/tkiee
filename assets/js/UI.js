$(function () {
    ...
    function scaleSlider(width, height) {
        ...
        if (window.innerWidth * (imgHeight / imgWidth) + 21 > windowHeight) {
            ...
            setWidthSlides = window.innerWidth;  
            setHeightSlides = window.innerWidth * (imgHeight / imgWidth);                      
        }
        setWidthContainer = setWidthSlides;
        setHeightContainer = setHeightSlides + 21;


/*        if (imgWidth / imgHeight > (window.innerWidth - 100) / (windowHeight - 21)) {
            setWidthSlides = window.innerWidth - 100;
            setHeightSlides = (window.innerWidth - 100) * (imgHeight / imgWidth);
        } else if (imgWidth / imgHeight == (window.innerWidth - 100) / (windowHeight - 21)) {
            setWidthSlides = window.innerWidth - 100;
            setHeightSlides = (windowHeight - 21);
        } else {
            setWidthSlides = (windowHeight - 21) * (imgWidth / imgHeight);
    ...
    var loop = setInterval(function () {
        if (typeof jssor_slider1 != "undefined" && typeof pageData.session.slides != "undefined") {
            jssor_slider1.$On($JssorSlider$.$EVT_PARK,function(slideIndex, fromIndex){
                $("#currentFilePage").text("File: " + pageData.session.slides[slideIndex].name + " : Page " + (pageData.session.slides[slideIndex].page + 1) + "/" + pageData.session.slides[slideIndex].totalPage);
            });
